...
const { INIComment } = require("./atom/inicomment");
const { INIPosition } = require("./position/iniposition");
const { INISectionHeader } = require("./atom/inisectionheader");
const { INIEmpty } = require("./atom/iniempty");
const { INIKVPair } = require("./atom/inikvpair");
...
class INIFileParser {
    static async parseFileToINIObject(iniFile) {
        ...
        for (let item of content) {
            let originLine = checkObjectNull(item) ? "" : item;
            ...
            if (trimmedLine.startsWith(";")) {
                ...
                let valueNameSB = new StringBuilder();
    ...
    static checkSameLine(preINIContent, curINIContent) {
        if (
            !checkObjectNull(preINIContent) &&
            !checkObjectNull(curINIContent)
        ) {
            let prePosition = preINIContent.getPosition();
            let curPosition = curINIContent.getPosition();
            if (checkObjectNull(prePosition) || checkObjectNull(curPosition)) {
                return false;
            }
